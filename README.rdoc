...
these can contain secret tokens for environment vars. e.g. for twitter oauth.

start with `foreman start -f Procfile.dev -e .env.dev`
...
run `heroku config:set TERM_CHILD=1` to set up the right signal handling

for heroku, add the following to the `Procfile`:
